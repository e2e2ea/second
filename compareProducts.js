...
const jsonArrays = ['Freezer-Frozen Seafood-Frozen Seafood.json']
const getData = async () => {
    let productsMatched = []
    await dbConnect();
    for (const jsonArray of jsonArrays) {
        ...
        for (const data of jsonData) {
            const products = await Product.find({ category: 'Freezer', subCategory: 'Frozen Seafood' });
        ...
        try {
            ...
            const fileName = `Freezer - Frozen Seafood - Frozen Seafood.json`;
