...
const jsonArrays = ['Drinks-Tea-Herbal & Specialty Tea.json']
const getData = async () => {
    let productsMatched = []
    await dbConnect();
    for (const jsonArray of jsonArrays) {
        ...
        for (const data of jsonData) {
            const products = await Product.find({ category: 'Drinks', subCategory: 'Tea' });
        ...
        try {
            ...
            const fileName = `Drinks - Tea - Herbal & Specialty Tea.json`;
